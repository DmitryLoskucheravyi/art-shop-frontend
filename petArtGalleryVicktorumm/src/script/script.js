...
window.addEventListener('DOMContentLoaded', () => {
    ...
    class Art {
        ...
        render() {
            ...
            <ul class="item-info-list">
              ...
              <li class="item-info-list-el uk hide" style="font-weight: 600;">${this.paintTypeUa}</li>
    ...
    const getArtCards = async (url) => {
        ...
        if(!res.ok){
            throw new Error(`Couldnt fetch are product's of ${url}, error:${res.status}`)
        }
        return await res.json()
    }


    getArtCards('http://localhost:3000/artDB')
    .then(data => data.forEach(({id, title, artSize, paintType, paintTypeUa, year, img, priceUSD, priceUAH}) => {
        new Art(id, title, artSize, paintType, paintTypeUa, year, img, priceUSD, priceUAH).render();
    }))
    ...
    function BasketRender() {
        ...
        <div class="order-item-title" data-delete="${id}">
            ...
            <img src="assets/icons/order-item-delete.svg" alt="close-icon" class="order-item-title-delete" data-delete>
        </div>
